#!/usr/bin/env bash
# release.sh — Build, sign, and upload a release APK for @carehub/kiosk
#
# Usage:
...
#      keytool -genkey -v -keystore carehub-kiosk.keystore -alias kiosk -keyalg RSA -keysize 2048 -validity 10000
#   2. Create packages/kiosk/.env.release with:
...
if [[ -n "${KEYSTORE_PATH:-}" && ! -f "$KEYSTORE_PATH" ]]; then
  ...
  echo "Generate it with: keytool -genkey -v -keystore carehub-kiosk.keystore -alias kiosk -keyalg RSA -keysize 2048 -validity 10000" >&2
  PREREQ_OK=false
fi
